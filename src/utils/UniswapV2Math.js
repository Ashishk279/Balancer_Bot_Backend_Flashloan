...
export class UniswapV2Math {
     ...
     */
    // static async getAmountOut(amountIn, reserveIn, reserveOut) {
    //     try {
    //         // Convert to Decimal for precise calculations
    //         const amountInDecimal = new Decimal(amountIn);
    //         const reserveInDecimal = await new Decimal(reserveIn);
    //         const reserveOutDecimal = new Decimal(reserveOut);
    //         // console.log("amountInDecimal: " + amountInDecimal);
    //         // console.log("reserveInDecimal: " + reserveInDecimal + " reserveOutDecimal: " + reserveOutDecimal);
    ...
    try {
        // Inputs are already BigInts from the main script.
        // Convert to Decimal for precise calculations.
        ...
        // Use BigInts directly in the calculation for optimal performance.
        const amountInWithFee = amountIn * 997n;
        const numerator = amountInWithFee * reserveOut;
        const denominator = reserveIn * 1000n + amountInWithFee;
        const amountOut = numerator / denominator;
        
        // Return the amount as a BigInt
        return amountOut;
    } catch (error) {
        // You can log errors here if needed
        console.error('Error in getAmountOut calculation', error.message);
        throw error;
    }
}
